...
public class OrdenCompra extends InfoComprasUIA  {
    ...
    @JsonCreator
    public OrdenCompra(@JsonProperty("id")int id, @JsonProperty("name")String name,
    ...
    public void agregaItems(ListaReportesNivelStock miReporteNS)
    {
        ...
        for(int i=0; i<miReporteNS.getItems().size(); i++)
        {
            ...
            {
                nodo = new OrdenCompra(miNodo.getId(), miNodo.getName(), miNodo.getDescripcion(),
                        "PZA", miNodo.getPedidoProveedor(), "itemOC");
